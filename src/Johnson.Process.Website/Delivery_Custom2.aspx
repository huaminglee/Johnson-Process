<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Delivery_Custom2.aspx.cs" Inherits="Johnson.Process.Website.Delivery_Custom2" %>
...
<script language="javascript" type="text/javascript">
    ...
    $(function () {
        ...
        $.get("DeliveryController.aspx?action=get", { taskId: taskId, r: Math.random() }, function (data) {
            ...
            $("#csdReplyForm").setFormValue(data);
            $("#logReplyForm").setFormValue(data).setFormReadOnly(true);
        });

        $("#csdReplyForm,#remarkForm").validate();
        ...
        $("#btnSubmit").button().click(function () {
            if (!$("#csdReplyForm").validAndFocus()) {
                return;
            }
            var replyObj = $.getFormValue("#csdReplyForm");
            var remarkObj = $.getFormValue("#csdReplyForm");
            var csdReply = replyObj.csdReply;
            ...
            $.post("DeliveryController.aspx?action=Delivery_Custom2_Submit", { taskId: taskId, csdReply: csdReply, submitRemark: submitRemark }, function (data) {
                if (data.result != 0) {
                    alert(data.message);
                }
                else {
        ...
        $("#btnReturn").button().click(function () {
            ...
            $(this).attr("disabled", "disabled");
            $.post("DeliveryController.aspx?action=Delivery_Custom2_Return", { taskId: taskId, csdReply: csdReply, submitRemark: submitRemark }, function (data) {
                if (data.result != 0) {
                    alert(data.message);
                }
                else {
