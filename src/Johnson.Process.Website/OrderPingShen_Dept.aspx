<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OrderPingShen_Dept.aspx.cs" Inherits="Johnson.Process.Website.OrderPingShen_Dept" %>
...
<body>
    <%--head --%>
    <johnson:Header runat="server" HeaderTitle="订单评审-组织评审" ID="header"></johnson:Header>
    ...
    <form id="deptPingShenForm">
        <table class="formInfo">
            <tr>
                <td class="labelCol" style="width: 200px">
                    评审结果<span style="color: Red" >*</span>
                </td>
                <td colspan="3" class="textCol">
                    <textarea name="result" class="textInput required" style="width: 600px;" rows="3"></textarea>
                </td>
            </tr>
        </table>
    </form>
...
<script language="javascript" type="text/javascript">
    ...
    $(function () {
        ...
        $("#btnSubmit").button().click(function () {
            if (!$("#deptPingShenForm").validAndFocus()) {
                return;
            }
            var valueObj = $.getFormValue("#deptPingShenForm, #remarkForm");

            if (!confirm("您确实要提交吗？")) {
                return;
            }
            $(this).attr("disabled", "disabled");
            $.post("OrderPingShenController.aspx?action=DeptPingShenSubmit", { taskId: taskId, submitRemark: valueObj.submitRemark, resut: valueObj.result }, function (data) {
                if (data.result != 0) {
                    alert(data.message);
                }
                else {
