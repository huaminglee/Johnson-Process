<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="VOC_Measures.aspx.cs" Inherits="Johnson.Process.Website.VOC_Measures" %>
...
<body>
    <%--head --%>
    <johnson:Header runat="server" HeaderTitle="VOC-纠正预防措施" ID="header"></johnson:Header>
       ...
       <table class="formInfo">
            <tr >
                ...
                <td class="textCol" colspan="3" >
                    <textarea name="reason" class="textInput required" readonly="readonly" style="width: 688px;" rows="5"></textarea>
                </td>
            </tr>
        </table>
...
<script language="javascript" type="text/javascript">
    ...
    $(function () {
        $.get("VocController.aspx?action=get", { taskId: taskId, r: Math.random() }, function (data) {
            $("#basicInfoForm, #reasonAndMeasuresForm").setFormValue(data);
        ...
        $("#btnSubmit").button().click(function () {
            ...
            $.post("VocController.aspx?action=ResponsibleMeasuresSubmit", { formJson: argsJson }, function (data) {
                if (data.result != 0) {
                    alert(data.message);
                }
                else {
        ...
        $("#reasonAndMeasuresForm").validate();

        $("#remarks, #actionGrid, #attachments, #reasonAttachments, #solutionsAttachments").datagrid({
            rownumbers: true
        });
    })
</script>