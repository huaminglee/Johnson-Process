<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Voc_ResponsibleReason.aspx.cs" Inherits="Johnson.Process.Website.Voc_ResponsibleReason" %>
...
<body>
    <%--head --%>
    <johnson:Header runat="server" HeaderTitle="VOC-原因分析" ID="header"></johnson:Header>
    ...
    <br />
    <form id="responnsibleMeasuresForm">
        <table class="formInfo">
            <tr>
...
<script language="javascript" type="text/javascript">
    ...
    $(function () {
        $.get("VocController.aspx?action=get", { taskId: taskId, r: Math.random() }, function (data) {
            $("#basicInfoForm, #responnsibleMeasuresForm, #reasonAndMeasuresForm").setFormValue(data);
        ...
        $("#btnSubmit").button().click(function () {
            if (!$("#responnsibleMeasuresForm").validAndFocus()) {
                return;
            }
            ...
            var responnsibleUserPrevious = $("#responnsibleMeasuresForm").getFormValue();
            args.measureUserName = responnsibleUserPrevious.measureUserName;
            args.measureUserAccount = responnsibleUserPrevious.measureUserAccount;
            ...
            $.post("VocController.aspx?action=ResponsibleReasonSubmit", { formJson: argsJson }, function (data) {
                if (data.result != 0) {
                    alert(data.message);
                }
                else {
        ...
        $("#responnsibleMeasuresForm, #reasonAndMeasuresForm").validate();

        $("#remarks, #actionGrid, #attachments, #solutionsAttachments").datagrid({
            rownumbers: true
        });
    })
</script>